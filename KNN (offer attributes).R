...
data$offer_type <- as.factor(data$offer_type)
data$channels <- as.factor(data$channels)

data_train <- data[1:floor(0.8 * nrow(data)), ]
data_test <- data[(floor(0.8 * nrow(data)) + 1):nrow(data), ]

train <- cbind(data_train$gender, data_train$age, data_train$income, data_train$membership_year, data_train$offer_type, data_train$difficulty, data_train$duration, data_train$channels)
test <- cbind(data_test$gender, data_test$age, data_test$income, data_test$membership_year, data_test$offer_type, data_test$difficulty, data_test$duration, data_test$channels)
